...
class SessionClient(BaseClient):
    """
    Session based API client - used for login/password authentication 
    (for example for background tasks)
    """

    def __init__(self, odesk_login, odesk_password, session_id=None, 
                 format='json'):
        self.odesk_login = odesk_login
    ...
    def login(self):
        url = self.base_url + 'auth/v%s/login.json' % self.version
        data = {'username': self.odesk_login, 
                'password': self.odesk_password}
        
        try:
            result, cj = urlload(url, data=data, method='POST')
        except urllib2.HTTPError, e:
            if e.code == 401:
